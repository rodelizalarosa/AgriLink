...
const ProductCard: React.FC<ProductCardProps> = ({ product, onClick }) => {
  ...
  return (
    ...
    >
      ...
      <div className="absolute bottom-0 left-0 translate-y-1/2 -translate-x-1/2 w-24 h-24 bg-[#4d8f08]/5 rounded-full blur-2xl pointer-events-none group-hover:bg-[#4d8f08]/10 transition-colors" />

      {/* Floating Leaves */}
      <Leaf className="absolute top-1/2 left-4 w-4 h-4 text-[#5ba409]/10 -rotate-12 pointer-events-none group-hover:text-[#5ba409]/20 transition-all duration-700 delay-100 group-hover:translate-y-[-20px] group-hover:rotate-12" />
      <Leaf className="absolute bottom-1/4 right-4 w-6 h-6 text-[#5ba409]/5 rotate-45 pointer-events-none group-hover:text-[#5ba409]/15 transition-all duration-700 group-hover:translate-y-[-10px] group-hover:-rotate-12" />
      {/* 🖼️ Image Container */}
      <div className="relative h-64 overflow-hidden bg-gray-50">
        ...
        <div className="absolute bottom-5 left-5 right-5 z-20 flex items-center justify-between">
          ...
          <div className="bg-white px-3 py-2 rounded-2xl flex items-center gap-1 shadow-2xl">
            <span className="text-[10px] font-black text-[#5ba409] uppercase tracking-wider">{product.stock} {product.unit} left</span>
          </div>
        </div>
      </div>
