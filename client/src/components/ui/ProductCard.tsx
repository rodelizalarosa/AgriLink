...
const ProductCard: React.FC<ProductCardProps> = ({ product }) => {
  return (
    <div className="bg-white rounded-2xl shadow-md hover:shadow-2xl transition-all duration-300 overflow-hidden group border-2 border-transparent hover:border-[#4CAF50] transform hover:-translate-y-1">
      ...
      <div className="p-5">
        ...
        <div className="flex items-center text-sm text-gray-600 mb-4">
          <MapPin className="w-4 h-4 mr-1 text-[#4CAF50]" />
        ...
        <div className="flex items-center justify-between pt-3 border-t-2 border-gray-100">
          ...
          <button className="bg-[#4CAF50] hover:bg-[#45A049] text-white px-5 py-2 rounded-full font-semibold shadow-md hover:shadow-lg transform hover:scale-105 transition-all flex items-center space-x-1">
            <ShoppingCart className="w-4 h-4" />
            <span>Add</span>
          </button>
        </div>
        <div className="mt-3 text-sm text-gray-600">
          <span className="font-semibold text-[#4CAF50]">{product.stock} {product.unit}</span> available
        </div>
      </div>
    </div>
  );
