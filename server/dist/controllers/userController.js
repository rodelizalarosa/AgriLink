...
exports.getAllFarmers = exports.updateUserProfile = exports.getUserProfile = exports.updateOnboardingStatus = void 0;
...
const updateOnboardingStatus = async (req, res) => {
    try {
       ...
       WHERE id = ?`, [phone, address, city, province, zip_code, latitude, longitude, userId]);
...
const updateUserProfile = async (req, res) => {
    try {
        ...
        if (userUpdates.length > 0) {
         ...
         WHERE id = ?`, [...userUpdates.map((u) => u.value), userIdValue]);
        }
        ...
        if (userColumns.has('role')) {
            const [roleRows] = await database_1.db.execute('SELECT role FROM users_table WHERE id = ? LIMIT 1', [userIdValue]);
            userRole = String(roleRows?.[0]?.role || '').toLowerCase();
        }
        else {
            const [roleRows] = await database_1.db.execute(`SELECT r.role_name
         FROM users_table u
        ...
        const hasFarmInput = safeFarmName !== null ||
        ...
        if (hasFarmsTable && (userRole === 'farmer' || hasFarmInput)) {
            ...
            const farmValues = [userIdValue, ...farmEntries.map((entry) => entry.value)];
            const farmUpdates = farmEntries.length > 0
         ...
         ON DUPLICATE KEY UPDATE ${farmUpdates.join(', ')}`, farmValues);
           ...
           WHERE id = ?`, [...legacyFarmUpdates.map((entry) => entry.value), userIdValue]);
            }
        }
