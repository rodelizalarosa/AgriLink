...
export const getAllProducts = async () => {
    const [rows] = await db.execute(`
        SELECT p.*, c.cat_name, u.first_name, u.last_name
...
export const createProduct = async (productData: any) => {
    ...
    // Restriction: Only 1 active listing per crop (p_name) per farmer (u_id)
...
export const updateProduct = async (pId: number, productData: any) => {
    ...
    console.log('--- productService.updateProduct Parameters ---');
    console.log('pId:', pId);
    console.log('Params:', [p_name, p_description, p_price, p_unit, p_quantity, p_category, p_image, p_status, harvest_date]);
...
export const archiveProduct = async (pId: number, uId: number) => {
    // Restriction: Cannot archive crop with active or reserved orders (Pending or Confirmed)
